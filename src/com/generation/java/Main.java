...
public class Main
{
    ...
    private static void gradeStudent( StudentService studentService, Scanner scanner )
    {
        ...
        //TODO print out student's list of enrolled courses
        String courseStr = studentService.printCoursesList(student);
        if (courseStr.isEmpty())
            System.out.println("\nStudent did not enroll for any course");
        else {
            System.out.println(courseStr);
            System.out.println("\nInsert Course ID to be graded: ");
            String courseID = scanner.next();
            Course course = student.findCourseById(courseID);
            //Checking if courseID is valid, if not, user to key in again
            while (course == null) {
                System.out.println("\nInvalid Course ID, Please enter again: ");
                courseID = scanner.next();
                course = student.findCourseById(courseID);
            }
            ...
            while (grade < 0 || grade > 6) {
                System.out.println("\nInvalid grade. Please enter a grade from 1.0 to 6.0: ");
